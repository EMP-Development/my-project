...
class ExcelMapperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel転記ツール v3.7（ログ差異色付け版）")
    ...
    def execute_mapping(self):
        try:
            ...
            src_key_idx, dst_key_idx = self.col_to_index(src_key_col), self.col_to_index(dst_key_col)
            dst_dict = {}
            ...
            for i in range(src_start - 1, len(df_src)):
                ...
                for s, d in mappings:
                    ...
                    dst_before = df_dst.iat[dst_i, d_idx] if d_idx < len(df_dst.columns) else None
                    ...
                    log_colors.append((dst_after, src_val, dst_before))

                for col_idx, v in enumerate(log_values, start=1):
                    ws_log.cell(row=log_row, column=col_idx, value=v)

                # 差異がある場合、対象列を赤く塗る
                for j, (dst_after, src_val, dst_before) in enumerate(log_colors):
                    diff = (src_val != dst_before)
                    if diff:
                        dst_col_pos = 4 + j * 2
                        src_col_pos = 5 + j * 2
                        if mode == "上書き":
                            ws_log.cell(row=log_row, column=src_col_pos).fill = red_fill
                        elif mode == "既存保持":
                            ws_log.cell(row=log_row, column=dst_col_pos).fill = red_fill
                        elif mode == "確認のみ":
                            ws_log.cell(row=log_row, column=dst_col_pos).fill = red_fill
                            ws_log.cell(row=log_row, column=src_col_pos).fill = red_fill
