...
class ExcelMapperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel データ抽出・転記ツール（実用安定版）")
    ...
    def execute_mapping(self):
        try:
            ...
            ws = wb.active
            ws["A1"] = "転記元ファイル"
            ws["B2"] = src_path.split("/")[-1]
            ws["B3"] = src_sheet
            ws["A4"] = "転記先ファイル"
            ws["B5"] = dst_path.split("/")[-1]
            ws["B6"] = dst_sheet
            ...
            try:
                with pd.ExcelWriter(dst_path, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                    dst_df.to_excel(writer, sheet_name=dst_sheet, index=False, header=False)
            except Exception:
