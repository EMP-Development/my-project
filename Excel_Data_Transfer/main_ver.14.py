...
class ExcelMapperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel転記ツール v3.8（色分け・差異判定強化版）")
    ...
    def execute_mapping(self):
        try:
            ...
            # === 色設定 ===
            pale_red = PatternFill(start_color="F8CBAD", end_color="F8CBAD", fill_type="solid")
            pale_yellow = PatternFill(start_color="FFF2CC", end_color="FFF2CC", fill_type="solid")
            ...
            for i in range(src_start - 1, len(df_src)):
                ...
                for s, d in mappings:
                    ...
                    if mode == "上書き":
                        ...
                        if different and dst_empty:
                            ws_dst.cell(row=dst_excel_row, column=d_idx + 1).value = src_val
                            applied = True
                        elif different and src_empty:
                            color_type = "pale_red"
                    elif mode == "確認のみ":
                        applied = False
                        if different:
                            color_type = "pale_yellow"
                        elif src_empty or dst_empty:
                            color_type = "pale_red"

                    if different or mode == "確認のみ":
                        diff_found = True

                    log_values += [dst_val if not applied else src_val, src_val]
                ...
                for col_idx, v in enumerate(log_values, start=1):
                    ...
                    if col_idx > 3:
                        color = color_marks[(col_idx - 4) // 2] if (col_idx - 4) // 2 < len(color_marks) else None
                        if color == "pale_red":
                            cell.fill = pale_red
