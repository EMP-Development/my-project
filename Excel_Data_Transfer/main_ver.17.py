...
class ExcelMapperApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Excel転記ツール v3.11（書き換え前後ログ）")
    ...
    def execute_mapping(self):
        try:
            ...
            for s, d in mappings:
                header += [f"転記先{d}_書き換え前", f"転記先{d}_書き換え後", f"転記元{s}"]
            ...
            for i in range(src_start - 1, len(df_src)):
                ...
                for s, d in mappings:
                    ...
                    dst_val_before = df_dst.iat[dst_i, d_idx] if d_idx < len(df_dst.columns) else None

                    src_empty = pd.isna(src_val) or src_val == ""
                    dst_empty = pd.isna(dst_val_before) or dst_val_before == ""
                    different = src_val != dst_val_before
                    ...
                    dst_val_after = ws_dst.cell(row=dst_excel_row, column=d_idx + 1).value
                    ...
                    log_values += [dst_val_before, dst_val_after, src_val]
