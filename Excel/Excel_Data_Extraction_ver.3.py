...
# Excel抽出処理
def extract_data_from_excels(input_dir, output_file, target_col_letter, search_list,
...
def run_gui():
    ...
    search_entry.bind("<Return>", on_enter)  # Enterキーでも追加
    tk.Button(root, text="追加", command=add_search_item).grid(row=3, column=2, padx=5, pady=5)

    # 検索文字列一覧
    tk.Label(root, text="検索文字列一覧:").grid(row=4, column=0, sticky="ne", padx=5, pady=5)
